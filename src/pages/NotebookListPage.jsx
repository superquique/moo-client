...
import { FolderIcon, FolderPlusIcon, TrashIcon } from "@heroicons/react/24/outline";
...
function NotebookListPage () {
    ...
    const addNotebook = (name) => {
        notebooksService.createNotebook({name: name})
        .then((response) => {
            getNotebooks();
        })
        .catch((error) => console.log(error));
    ...
    const onOpenForm = () => document.getElementById('add-notebook-modal').showModal()
    ...
    return (
        ...
        <div>
            <AddNotebookForm onAddNotebook={addNotebook} />
            <ul className="list bg-base-100 rounded-box shadow-md">
  
                <li className="p-4 pb-2 text-xs opacity-60 tracking-wide">
                    Notebooks

                    <button onClick={onOpenForm} className="btn btn-square btn-ghost">
